...
# لیست جامع ۱۳ دارایی با نمادهای تضمین شده TradingView
assets = {
    ...
    'SP500': {'ticker': '^GSPC', 'tv_symbol': 'TVC:SPX'},
    'NSDQ100': {'ticker': '^IXIC', 'tv_symbol': 'TVC:NDX'},
    'RTY2000': {'ticker': '^RUT', 'tv_symbol': 'TVC:RTY'},
    'AUS200': {'ticker': '^AXJO', 'tv_symbol': 'TVC:AS200'},
    'CHINA50': {'ticker': 'FXI', 'tv_symbol': 'FX_IDC:CN50'},
    'JAPAN225': {'ticker': '^N225', 'tv_symbol': 'TVC:NI225'},
    'GERMANY40': {'ticker': '^GDAXI', 'tv_symbol': 'TVC:DAX'},
    'UK100': {'ticker': '^FTSE', 'tv_symbol': 'TVC:UKX'}
}

def generate_dashboard():
    # ثبت دقیق زمان ملبورن
        ...
        try:
            ...
            <div onclick="changeChart('{info['tv_symbol']}')" class="asset-card">
                ...
                <div class="stats-grid">
                    ...
                    <div class="stat-item"><span>حد سود:</span><span class="val" style="color:#10b981">{take_profit:,.2f}</span></div>
                    <div class="stat-item"><span>حد ضرر:</span><span class="val" style="color:#ef4444">{stop_loss:,.2f}</span></div>
                </div>
            </div>"""
        except: continue
    ...
    <head>
        <meta charset="UTF-8">
        <style>
            body {{ background:#0a0f1e; color:white; font-family:Segoe UI, Tahoma; margin:0; padding:15px; }}
            .header {{ text-align:center; margin-bottom:15px; border-bottom:1px solid #1e293b; padding-bottom:10px; }}
            .container {{ display:grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap:10px; margin-bottom:15px; }}
            ...
            .asset-header {{ display:flex; justify-content:space-between; font-size:10px; font-weight:bold; margin-bottom:5px; }}
            .asset-name {{ color:#b4b7bd; }}
            .asset-price {{ font-size:17px; font-weight:bold; margin-bottom:8px; }}
    ...
    <body>
        ...
        <script>
            ...
            changeChart('TVC:GOLD'); // نمودار پیش‌فرض
        </script>
    </body>
    </html>"""

    with open(HTML_FILE, "w", encoding="utf-8") as f: f.write(full_html)
    print(f"✅ داشبورد با موفقیت در ساعت {now_time} آپدیت شد.")
